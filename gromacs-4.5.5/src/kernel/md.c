             ...
             gmx_runtime_t *runtime)
{
   ...
   double *ener2;
   double *vol2;
   int    i,stepper=0,stepper2=0;
   double *av_x,*av_y,*av_z,msd2;
   double rms2;
   double av_pres, var_pot,var_vol;
   double av2,av2b;
   float  global_exchange;
   ...
   printf("%d\t%s\n",dend,"end of sequence");
    ...
    snew(av_y,dend+1);
    snew(av_z,dend+1);
    snew(current_x,dend+1);
    snew(current_y,dend+1);
    snew(current_z,dend+1);
    snew(pres2,repl_ex_nst+1);
    snew(ener2,repl_ex_nst+1);
    snew(vol2,repl_ex_nst+1);
...
//	printf("%e\t%e\t%e\t%s\n",pres2[stepper],ener2[stepper],vol2[stepper]);
       ...
       if(MASTER(cr))
            ...
            pres2[stepper] = enerd->term[F_PRES];
            ener2[stepper] = enerd->term[F_EPOT];
            vol2[stepper]  = det(state->box);                
                ...
                for(i=0;i<dend;i++){
                        ...
                        if(step > 0) rms2 = rms2 + 1/(double)(dend-dstart)*(pow((current_x[i] - av_x[i]),2)+pow((current_y[i]-av_y[i]),2)+pow((current_z[i] - av_z[i]),2));
	...
	//	printf("%d\n",stepper);		   
            ...
            do_per_step(step,repl_ex_nst)){
                     ...
                     for(i = 1; i<= stepper; i++){
                       ...
                       av2 += ener2[i]/(double)stepper;
                       av2b += pow(ener2[i],2)/(double)stepper;

                     };

                     var_pot = sqrt(av2b - pow(av2,2));
                     ...
                     for(i = 1; i<= stepper; i++){
                       ...
                       av2 += vol2[i]/(double)stepper;
                       av2b += pow(vol2[i],2)/(double)stepper;
		       
		  //     printf("%e\t%e\t%e\n",vol2[i],av2,av2b);

                     };
		     
                     var_vol = sqrt(av2b - pow(av2,2));

                     av2 = 0.0;

                     for(i = 1; i<= stepper; i++){

                       av2 += pres2[i]/(double)stepper;

                     };

                     av_pres = av2;

		//     printf("%e\t%e\t%e\t%e\n",rms2,var_pot,var_vol,av_pres);
            ...
            do_per_step(step,repl_ex_nst)) 
        {
            ...
            bExchanged = replica_exchange(fplog,cr,repl_ex,
                                          ...
                                          state,step,t,Rstep,var_pot,var_vol,av_pres,rms2);
